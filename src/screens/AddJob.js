...
import {addJob, updateJob} from "../rtk/features/todoListSlice"
import { useDispatch, useSelector } from 'react-redux'

const AddJob = ({route, navigation}) => {
    let dispatch = useDispatch()
    let id = useSelector((state) => state.todoListSlice.list.length) + 1
    ...
    useEffect(() => {
        ...
        setNameJob(route.params.job.job)
        setIsUpdate(route.params.isUpdate)
    }, [JSON.stringify(route.params)])

    let handlePressBtnFinish = () => {
        if(isUpdate){
            dispatch(updateJob({id : job.id, job : nameJob, isChecked : false}))
        }
        else{
            dispatch(addJob({id : id, job : nameJob, isChecked : false}))
        }
